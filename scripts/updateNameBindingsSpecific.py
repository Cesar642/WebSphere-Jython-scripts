...
def updateKeys(toUpdate):
  for ns in AdminConfig.list( 'NameSpaceBinding' ).splitlines() :
    ...
    #print "**ns: %s\n" % (ns)
    #findKey(nameInNameSpace)
    if (findKey(ns)):
      if (whatToUpdate == "stringToBind"):
        print "Update stringToBind"
        params = "[[name " + name + "] [nameInNameSpace " + nameInNameSpace + "] [stringToBind " + nameDic[ns] + "]]"
      elif (whatToUpdate == "nameInNameSpace"):
        print "Update nameInNameSpace"
        params = "[[name " + name + "] [nameInNameSpace " + nameDic[ns] + "] [stringToBind " + stringToBind + "]]"
      elif (whatToUpdate == "name"):
        print "Update name"
        params = "[[name " + nameDic[ns] + "] [nameInNameSpace " + nameInNameSpace + "] [stringToBind " + stringToBind + "]]"
      typeToEdit = ns
